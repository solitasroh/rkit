...
// NOTE (bkit-gstack-sync-v2 / Cycle 1, C4):
//   common.js / context-hierarchy.js were removed in bkit v2.1.0 S1 cleanup.
//   We migrate to a single lazy `core` barrel for PLUGIN_ROOT / PROJECT_DIR /
//   debugLog and inline getUserConfigDir() here.
...
function loadImportedContent(absolutePath) {
  ...
  try {
    if (!fs.existsSync(absolutePath)) {
      debugLog('Import file not found', { path: absolutePath });
      return '';
    }
    ...
    debugLog('Failed to load import', { path: absolutePath, error: e.message });
    return '';
  }
}
