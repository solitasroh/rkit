 ...
 * Also writes profile.md for direct agent imports.
 */
...
/**
 * Write converged patterns to .rkit/instinct/profile.md for agent imports.
 * Called at SessionStart so agents can reference the file directly.
 * @returns {string} Path to the written profile file
 */
function writeProfileFile() {
  try {
    const text = loadConvergedPatterns();
    const instinctBase = store.getInstinctBase();
    if (!fs.existsSync(instinctBase)) {
      fs.mkdirSync(instinctBase, { recursive: true });
    }
    const profilePath = path.join(instinctBase, 'profile.md');
    if (text) {
      const header = '<!-- Auto-generated by instinct loader. Do not edit manually. -->\n';
      const instruction = '\n\n> **Agent instruction**: Check these patterns FIRST when reviewing code. Report matches with `[INSTINCT]` tag.\n';
      fs.writeFileSync(profilePath, header + text + instruction, 'utf8');
    } else {
