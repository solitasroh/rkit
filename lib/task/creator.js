  ...
  if (['plan', 'design', 'do'].includes(phase)) {
    try {
      ...
      if (phase === 'design') {
        ...
        guidance += 'RULES for Mermaid:\n';
        guidance += '1. SUBGRAPHS: Group components into explicit architectural layers via `subgraph` (e.g., `subgraph Domain Layer`, `subgraph Presentation`).\n';
        guidance += '2. STEREOTYPES: Append GoF or structural pattern stereotypes to node names (e.g., `UserRepository<<Repository>>`, `LoginCtrl<<Controller>>`).\n';
